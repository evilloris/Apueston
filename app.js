...
function renderAll(){
  ...
  renderLeaderboard(); renderActiveEvents(); renderTournamentSelects(); renderBetMatches(); renderBetStandings();
...
async function finishMatch(id,walkover=false,side=null){
  ...
  await autoGenerateKnockout(m.tournament_id);
}

async function autoGenerateKnockout(tid){
  const t=tournamentById(tid);if(!t||!isGroupStageComplete(tid))return;
  const existing=tournamentMatches(tid).some(m=>m.phase!=="group");
  if(existing){renderKnockoutPanel();return}
...
async function generateKnockoutRows(tid){
  ...
  const repechage=$("#enableRepechage").checked;
  const third=$("#enableThirdPlace").checked;
  if(repechage){
    const extra=groups.map(g=>standingsFor(tid).filter(x=>x.group_no===g)[q]).filter(Boolean);
    if(extra.length>=2){
      await supabase.from("matches").insert({tournament_id:tid,phase:"repechage",round_no:1,side_a:extra[0].id,side_b:extra[1].id,status:"scheduled"});
      $("#bracketStatus").textContent="Repechaje creado. Las eliminatorias se completarán al finalizarlo.";
      return;
    }
  }
  ...
  if(third&&qualifiers.length>=4) t.config.third_place=true;
...
$("#enableRepechage").onchange=async()=>{
  const tid=$("#adminTournamentSelect").value,t=tournamentById(tid);if(!t)return;
  await supabase.from("tournaments").update({config:{...t.config,repechage:$("#enableRepechage").checked}}).eq("id",tid);await loadAll();$("#adminTournamentSelect").value=tid;renderKnockoutPanel();
};
$("#enableThirdPlace").onchange=async()=>{
  const tid=$("#adminTournamentSelect").value,t=tournamentById(tid);if(!t)return;
  await supabase.from("tournaments").update({config:{...t.config,third_place:$("#enableThirdPlace").checked}}).eq("id",tid);await loadAll();$("#adminTournamentSelect").value=tid;renderKnockoutPanel();
};
